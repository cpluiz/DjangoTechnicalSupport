...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if Category.objects.count() <= 0 :
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully populated db with default categories'))

        if mock_data:

            ThroughModel = User.groups.through

            if len(User.objects.filter(groups__name='Customers')) <= 0 :
                customers = [
            ...
            if len(User.objects.filter(groups__name='Attendants')) <= 1 :
                attendants = [
